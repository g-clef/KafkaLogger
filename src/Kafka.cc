...
bool KafkaWriter::DoInit(const WriterInfo& info, int num_fields, const threading::Field* const* fields)
{
	...
	if (strcmp("PLAINTEXT", security_protocol) != 0) {
		//Can be TLS for TLS only
		//SASL_PLAINTEXT for kerberos only
		//SASL_SSL for kerberos + ssl
		conf->set("security.protocol", security_protocol, errstr);
	}

	if ( (strcmp("SASL_SSL", security_protocol) == 0) || (strcmp("SASL_SSL", security_protocol) ==0) ){
